...
{
#ifndef SYMBIAN
        static PyObject *glob_implicit = NULL;
#else
#define glob_implicit (PYTHON_GLOBALS->implicit)
#endif
	...
	if (glob_implicit == NULL) {
		glob_implicit = PyInt_FromLong(1);
		if (glob_implicit == NULL)
			return -1;
	}
	v = NULL;
	...
	while (PyDict_Next(ste->ste_symbols, &pos, &name, &v)) {
		...
		*/
		...
		if (flags & DEF_STAR) {
			...
			si.si_nimplicit++;
			if (PyDict_SetItem(c->c_globals, name, glob_implicit) < 0)
				goto fail;
