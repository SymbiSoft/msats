...
{
  	// XXX:CW32
	PyBufferProcs *pb = (PyBufferProcs *)base->ob_type->tp_as_buffer;
	void *p;
	int count;
...
{
  	// XXX:CW32
	PyBufferProcs *pb = (PyBufferProcs *)other->ob_type->tp_as_buffer;
	char *p1;
	void *p2;
	PyObject *ob;
	int count;
...
buffer_ass_item(PyBufferObject *self, int idx, PyObject *other)
{
	...
	if (idx < 0 || idx >= self->b_size) {
  	...
  	// XXX:CW32
	pb = other ? (PyBufferProcs *)other->ob_type->tp_as_buffer : NULL;
	if ( pb == NULL ||
	     pb->bf_getreadbuffer == NULL ||
	     pb->bf_getsegcount == NULL )
	{
...
const static PySequenceMethods buffer_as_sequence = {
...
const static PyBufferProcs buffer_as_buffer = {
...
#else
const PyTypeObject c_PyBuffer_Type = {
