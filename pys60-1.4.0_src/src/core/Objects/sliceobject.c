...
#else
const PyTypeObject c_PyEllipsis_Type = {
...
#else
extern void _Py_EllipsisObject_Init()
{
  SPy_Python_globals* pyglobals = PYTHON_GLOBALS; // avoid TLS reads

#ifdef Py_TRACE_REFS
  (pyglobals->_Py_EllipsisObject)._ob_next = 0;
  (pyglobals->_Py_EllipsisObject)._ob_prev = 0;
#endif
  (pyglobals->_Py_EllipsisObject).ob_refcnt = 1;
  (pyglobals->_Py_EllipsisObject).ob_type = &PyEllipsis_Type;
...
const static PyMemberDef slice_members[] = {
...
PyTypeObject PySlice_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
#else
const PyTypeObject c_PySlice_Type = {
	PyObject_HEAD_INIT(NULL)
#endif
