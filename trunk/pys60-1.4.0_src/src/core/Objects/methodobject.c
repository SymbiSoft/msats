...
#else
#define free_list (PYTHON_GLOBALS->METHODOBJ_free_list)
#endif

// XXX:CW32
DL_EXPORT(PyObject *)
PyCFunction_New(const PyMethodDef *ml, PyObject *self)
{
	...
	// XXX:CW32
	op->m_ml = (PyMethodDef *)ml;
	Py_XINCREF(self);
	op->m_self = self;
	return (PyObject *)op;
}

DL_EXPORT(PyCFunction)
...
const static PyGetSetDef meth_getsets [] = {
...
#else
const PyTypeObject c_PyCFunction_Type = {
...
// XXX:CW32
DL_EXPORT(PyObject *)
Py_FindMethod(const PyMethodDef *methods, PyObject *self, char *name)
{
	PyMethodChain chain;
	// XXX:CW32
	chain.methods = (PyMethodDef *)methods;
	chain.link = NULL;
...
#undef free_list
#define free_list (pyglobals->METHODOBJ_free_list)
#endif
	while (free_list) {
