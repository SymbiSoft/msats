...
#else
const PyTypeObject c_PyType_Type = {
...
const static PyGetSetDef object_getsets[] = {
...
#else
#define copy_reg_str (PYTHON_GLOBALS->copy_reg_str)
#endif
...
const static PyMethodDef object_methods[] = {
...
PyTypeObject PyBaseObject_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
#else
const PyTypeObject c_PyBaseObject_Type = {
	PyObject_HEAD_INIT(NULL)
#endif
 	0,					/* ob_size */
	"object",				/* tp_name */
...
/* XXX:CW32 Ugly casting to avoid constness mismatch error. */
#define COPYSLOT(SLOT) \
	if (!type->SLOT && SLOTDEFINED(SLOT)) *(void **)&type->SLOT = (void *)(base->SLOT)
...
{ \
	return call_method(self, OPSTR, \
	                   &(PYTHON_GLOBALS->FUNCNAME##_cache_str), \
			   "()"); \
}
#else
#define SLOT0(FUNCNAME, OPSTR) \
...
{ \
	...
	if (self->ob_type->tp_as_number != NULL && \
		...
		if (do_other && \
		    PyType_IsSubtype(other->ob_type, self->ob_type)) { \
			r = call_maybe( \
				other, ROPSTR, &(PYTHON_GLOBALS->FUNCNAME##_rcache_str), "(O)", self); \
		...
		r = call_maybe( \
			self, OPSTR, &(PYTHON_GLOBALS->FUNCNAME##_cache_str), "(O)", other); \
		if (r != Py_NotImplemented || \
...
#else
#define len_str (PYTHON_GLOBALS->sq_length_len_str)
#endif
...
#else
#define getitem_str (PYTHON_GLOBALS->sq_item_getitem_string)
#endif
...
#else
#define delitem_str (PYTHON_GLOBALS->sq_ass_item_delitem_str)
#define setitem_str (PYTHON_GLOBALS->sq_ass_item_setitem_str)
#endif
...
#else
#define delslice_str (PYTHON_GLOBALS->sq_ass_slice_delslice_str)
#define setslice_str (PYTHON_GLOBALS->sq_ass_slice_setslice_str)
#endif
...
#else
#define contains_str (PYTHON_GLOBALS->contains_str)
#endif
...
slot_mp_ass_subscript(PyObject *self, PyObject *key, PyObject *value)
{
	...
	static PyObject *delitem_str, *setitem_str;
#else
#define delitem_str (PYTHON_GLOBALS->mp_ass_subs_delitem_str)
#define setitem_str (PYTHON_GLOBALS->mp_ass_subs_setitem_str)
#endif
	if (value == NULL)
		res = call_method(self, "__delitem__", &delitem_str,
				  "(O)", key);
...
#else
#define pow_str (PYTHON_GLOBALS->pow_str)
#endif
...
#else
#define nonzero_str (PYTHON_GLOBALS->nb_nonzero_str)
#define len_str (PYTHON_GLOBALS->nb_nonzero_len_str)
#endif
...
#else
#define coerce_str (PYTHON_GLOBALS->coerce_str)
#endif
...
#else
#define cmp_str (PYTHON_GLOBALS->half_compare_cmp_str)
#endif
	int c;
...
int _PyObject_SlotCompare(PyObject *self, PyObject *other)
{
	int c;
...
#else
#define repr_str (PYTHON_GLOBALS->repr_str)
#endif
...
#else
#define str_str (PYTHON_GLOBALS->str_str)
#endif
...
#else
#define hash_str (PYTHON_GLOBALS->tp_hash_hash_str)
#define eq_str (PYTHON_GLOBALS->tp_hash_eq_str)
#define cmp_str (PYTHON_GLOBALS->tp_hash_cmp_str)
#endif
...
#else
#define call_str (PYTHON_GLOBALS->call_str)
#endif
...
#else
#define getattribute_str (PYTHON_GLOBALS->o_getattribute_str)
#endif
	return call_method(self, "__getattribute__", &getattribute_str,
			   "(O)", name);
#ifdef SYMBIAN
#undef getattribute_str
#endif
}
...
#define getattribute_str (pyglobals->getattribute_str)
#define getattr_str (pyglobals->getattr_str)
#endif
	if (getattr_str == NULL) {
...
#else
#define delattr_str (PYTHON_GLOBALS->tp_set_delattr_str)
#define setattr_str (PYTHON_GLOBALS->tp_set_setattr_str)
#endif
...
static const char *const name_op[] = {
	"__lt__",
	"__le__",
	"__eq__",
	"__ne__",
...
#else
#define op_str (PYTHON_GLOBALS->op_str)
#endif

	// XXX:CW32
	func = lookup_method(self, (char *)name_op[op], &op_str[op]);
	if (func == NULL) {
		PyErr_Clear();
		Py_INCREF(Py_NotImplemented);
		return Py_NotImplemented;
...
const static int swapped_op[] = {Py_GT, Py_GE, Py_EQ, Py_NE, Py_LT, Py_LE};
...
#else
#define iter_str (PYTHON_GLOBALS->tp_iter_iter_str)
#define getitem_str (PYTHON_GLOBALS->tp_iter_getitem_str)
#endif
...
#else
#define next_str (PYTHON_GLOBALS->next_str)
#endif
...
#else
#define get_str (PYTHON_GLOBALS->tp_descr_get_str)
#endif
...
#else
#define del_str (PYTHON_GLOBALS->tp_descr_set_del_str)
#define set_str (PYTHON_GLOBALS->tp_descr_set_set_str)
#endif
	if (value == NULL)
...
#else
#define init_str (PYTHON_GLOBALS->init_str)
#endif
...
/* initialize slotdefs in RAM (shallow copy) */
extern int typeobject_globals_init()
{
    int i;
    int sz = 0;
    slotdef* ptemp;
    ...
    while(c_slotdefs[sz].name != NULL)
      sz++;

    pyglobals->slotdefs = (slotdef*) PyMem_Malloc((sz+1) * sizeof(slotdef));
    if (!(pyglobals->slotdefs))
      return (-1);

    for (i=0; i < (sz+1); i++) {
      ptemp = &((pyglobals->slotdefs)[i]);
      
      ptemp->name = c_slotdefs[i].name;
      ptemp->offset= c_slotdefs[i].offset;
      ptemp->function = c_slotdefs[i].function;
      ptemp->wrapper = c_slotdefs[i].wrapper;
      ptemp->doc = c_slotdefs[i].doc;
      ptemp->flags = c_slotdefs[i].flags;
      ptemp->name_strobj = c_slotdefs[i].name_strobj;
    }

    return 0;
}

extern void typeobject_globals_fini()
{
    PyMem_Free(PYTHON_GLOBALS->slotdefs);
}

#define slotdefs (pyglobals->slotdefs)
...
#else
	SPy_Python_globals* pyglobals = PYTHON_GLOBALS; // avoid TLS reads
#define initialized (pyglobals->slotdefs_initialized)
#endif
	if (initialized)
		return;
...
const static PyMemberDef super_members[] = {
...
PyTypeObject PySuper_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
#else
const PyTypeObject c_PySuper_Type = {
	PyObject_HEAD_INIT(NULL)
#endif
	0,					/* ob_size */
	"super",				/* tp_name */
