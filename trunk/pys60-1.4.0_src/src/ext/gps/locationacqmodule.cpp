 ...
 */
static TReal timeToReal(const TTime& time)
{
  TLocale locale;
  #ifndef EKA2
  return (((time.Int64().GetTReal()-epochToReal())/
          (1000.0*1000.0))-
          TInt64(locale.UniversalTimeOffset().Int()).GetTReal());
  #else
  return (((TReal64(time.Int64())-epochToReal())/(1000.0*1000.0)) 
...
new_Positioner_object(PositionServer_object* posServ, TPositionModuleId moduleUid)
{
  ...
  TRAP(err,{
    ...
    if(moduleUid==TPositionModuleId::Null()){
      // use default module.
      User::LeaveIfError(pos->Open(*(posServ->posServ)));
    }else{
      User::LeaveIfError(pos->Open(*(posServ->posServ),moduleUid));
    }
 ...
 */
extern "C" PyObject *
PositionServer_position(Positioner_object *self, PyObject* args)
{ 
  TInt flags=0;
  TInt err=KErrNone;
  ...
  TRAP(err,{
    		if(flags & SATELLITE_INFO)
    		{
      			positionInfo = new (ELeave) TPositionSatelliteInfo;
    		}
    		else if(flags & COURSE_INFO)
    		{
      			positionInfo = new (ELeave) TPositionCourseInfo;
    		}
    		else
    		{
      			positionInfo = new (ELeave) TPositionInfo;
    		}
  			}
  	)
  ...
  CPosDataFetcher* dataFetcher=NULL;
  TRAP(err,{
    dataFetcher=CPosDataFetcher::NewL(*(self->positioner));
    dataFetcher->FetchData(*positionInfo,status);
  });
  delete dataFetcher;
  ...
  if(flags & COURSE_INFO){
    TCourse course;
    static_cast<TPositionCourseInfo*>(positionInfo)->GetCourse(course);
  ...
  if(flags & SATELLITE_INFO){
    TPositionSatelliteInfo* satInfo = static_cast<TPositionSatelliteInfo*>(positionInfo);
...
extern "C" {
  ...
  const static PyMethodDef Positioner_methods[] = {
    ...
    {"position", (PyCFunction)PositionServer_position, METH_VARARGS, NULL},    
    {NULL, NULL, 0 , NULL} 
  };
