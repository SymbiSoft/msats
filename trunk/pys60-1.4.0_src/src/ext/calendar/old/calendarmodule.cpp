...
Entry_repeat_data(Entry_object* self, PyObject* /*args*/)
{
  ...
  value = 
    Py_BuildValue("d", time_UTC_as_UTC_TReal(self->entryItem->RptDef()->StartDate()));
  ADD_ITEM_TO_REP_DICT(key, value)
  ...
  if(self->entryItem->RptDef()->RepeatForever()!=EFalse){
    ...
    value = 
      Py_BuildValue("d", time_UTC_as_UTC_TReal(self->entryItem->RptDef()->EndDate()));
  }
  ADD_ITEM_TO_REP_DICT(key, value)
...
Entry_crossed_out_date(Entry_object* self, PyObject* /*args*/)
{
  ...
  return ttimeAsPythonFloatUtc(self->entryItem->CastToTodo()->CrossedOutDate());
}
...
Entry_last_modified(Entry_object* self, PyObject* /*args*/)
{
  ...
  return ttimeAsPythonFloatUtc(AgnDateTime::AgnDateTimeToTTime(dateTime));   
}
...
entryIterator_next(EntryIterator_object* self, PyObject* /*args*/)
{
  ...
  // Note that it must be checked if the entry is a deleted one which has positive
  // synchronization count and therefore has not been removed from the database.
  CAgnEntry* entry = NULL; 
  do{
    delete entry;
    entryId = self->calendarDb->agendaServer->EntryIteratorPosition();
    TRAP(err, entry = self->calendarDb->agendaModel->FetchEntryL(entryId));
    if(KErrNone != err){
      delete entry;
      return SPyErr_SetFromSymbianOSErr(err);
    } 
    self->hasMoreEntries = self->calendarDb->agendaServer->EntryIteratorNext();
  }while(entry->ReplicationData().HasBeenDeleted() && self->hasMoreEntries);
