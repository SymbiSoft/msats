...
extern "C" {
  ...
  {
    if (meso->smsObserver) {
      delete meso->smsObserver;
      meso->smsObserver = NULL;
    }
    if (meso->messaging) {
      delete meso->messaging;
      meso->messaging = NULL;
    }    
    if (meso->callBackSet) {
      Py_XDECREF(meso->myCallBack.iCb);
    }
    PyObject_Del(meso);
  }
...
new_mes_object(PyObject* /*self*/, PyObject* args)
{
  ...
  MES_object *meso = PyObject_New(MES_object, MES_type);
  if (meso == NULL)
    return PyErr_NoMemory();

  if (!PyArg_ParseTuple(args, "s#u#iO", &number, &number_l, &message, &message_l, &encoding, &c))  
    return NULL;
  ...
  TRAP(error, meso->messaging = CSmsSendHandler::NewL(*(meso->smsObserver), tel_number, msg_body, encoding));
