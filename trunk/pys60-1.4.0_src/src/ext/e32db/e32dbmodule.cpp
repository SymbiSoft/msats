...
/* Interpret the given long integer as a Symbian time value
   ...
   Unicode string, formatted according to the current
   locale. Surrounded with #-marks, the return value is suitable for
   use as a date/time literal in an SQL INSERT or UPDATE statement. */
/* AM/PM suffix is ignored if microseconds are present! 
  _LIT(KFormatTxt,"%/0%1%/1%2%/2%3%/3 %-B%:0%J%:1%T%:2%S%.%C%:3%+B"); */
